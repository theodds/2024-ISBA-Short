...
get_tau_bar <- function(fit) {
  num_iter <- nrow(fit$beta_train)
  num_obs <- ncol(fit$beta_train)
  omega <- MCMCpack::rdirichlet(n = num_iter, alpha = rep(1, num_obs))
  out_df <- data.frame(iter = 1:num_iter, tau_bar = rowSums(omega * fit$beta_train))
  return(out_df)
}
...
```{r}
post_sum <- rpart(colMeans(fitted_bcf$beta_train) ~ age + bmi + edu + income + povlev + 
                                    region + sex + marital + race + seatbelt,
                  data = meps)
rpart.plot(post_sum)
```

The most important indicators appear to be region and age, with (i) younger
people tending to have smaller causal effects and (ii) the Northeast and
South appearing to have larger/smaller effect. We first look at the causal
effects by region.

```{r}
get_region_effects <- function(fit) {
  regions <- unique(meps$region)
  iters <- nrow(fit$beta_train)
  out <- list()
  for(region in regions) {
    idx <- which(meps$region == region)
    tau_hat_region <- rowMeans(fit$beta_train[,idx])
    tau_bar <- rowMeans(fit$beta_train)
    out[[region]] <- data.frame(iteration = iters, region = region, Delta = tau_hat_region - tau_bar)
  }
  return(do.call(rbind, out))
}

region_effects <- get_region_effects(fitted_bcf)
ggplot(region_effects, aes(x = Delta, y = region, fill = region)) + 
  stat_halfeye()
```

Overall, there does not seem to be strong evidence that the overall effect
differs much across regions, and the ensemble turns out to only use region
for around 50% of the iterations.

# Sanity Check

```{r}
library(splines)
meps$smoke_hat <- fitted_smoke$p_train_mean

lm_check <- lm(phealth ~ smoke_res * (ns(age,5) + bmi + edu + income + povlev + 
                                    region + sex + marital + race + seatbelt), 
               data = meps)
tau_hat_lm <- predict(lm_check, meps %>% mutate(smoke_res = 1 - fitted_smoke$p_train_mean)) - 
  predict(lm_check, meps %>% mutate(smoke_res = 0 - fitted_smoke$p_test_mean))
hist(tau_hat_lm)
mean(tau_hat_lm)

lm_check <- lm(phealth ~ smoke * (ns(age,5) + bmi + edu + income + povlev + 
                                    region + sex + marital + race + seatbelt), 
               data = meps)
tau_hat_lm <- predict(lm_check, meps %>% mutate(smoke = 1)) - 
  predict(lm_check, meps %>% mutate(smoke = 0))
hist(tau_hat_lm)
mean(tau_hat_lm)

lm_check <- lm(phealth ~ (smoke_hat + smoke) * (ns(age,5) + bmi + edu + income + povlev + 
                                    region + sex + marital + race + seatbelt), 
               data = meps)
tau_hat_lm <- predict(lm_check, meps %>% mutate(smoke = 1)) - 
  predict(lm_check, meps %>% mutate(smoke = 0))
...
y <- meps$phealth
z <- meps$smoke
x_control <- model.matrix(phealth ~ age + bmi + edu + income + 
                                         povlev + region + sex + marital + race + 
                                         seatbelt - 1, data = meps)
pihat <- fitted_smoke$p_train_mean

fitted_bcf_2 <- bcf(y = y, z = z, x_control = x_control, pihat = pihat, 
...
tau_bar_df_2 <- get_tau_bar_2(fitted_bcf_2)

p_11 <- ggplot(tau_bar_df_2, aes(x = iter, y = tau_bar)) + geom_line() + xlab("Iteration") + ylab(TeX("\\bar{\\tau}")) + theme_bw()
p_21 <- ggplot(tau_bar_df_2, aes(x = tau_bar)) + stat_halfeye(.width = c(.90, .95, .99)) + ylab("Posterior Density") + xlab(TeX("\\bar{\\tau}"))

p_11 + p_21

get_region_effects_2 <- function(fit) {
  regions <- unique(meps$region)
  iters <- nrow(fit$tau)
  out <- list()
  for(region in regions) {
    idx <- which(meps$region == region)
    tau_hat_region <- rowMeans(fit$tau[,idx])
    tau_bar <- rowMeans(fit$tau)
    out[[region]] <- data.frame(iteration = iters, region = region, 
                                Delta = tau_hat_region - tau_bar)
  }
  return(do.call(rbind, out))
}

region_effects_2 <- get_region_effects_2(fitted_bcf_2)
ggplot(region_effects_2, aes(x = Delta, y = region, fill = region)) + 
  stat_halfeye()

rpart.plot(rpart(colMeans(fitted_bcf_2$tau) ~ age + bmi + edu + income + 
                                         povlev + region + sex + marital + race + 
                                         seatbelt, data = meps))

get_race_effects_2 <- function(fit) {
  regions <- unique(meps$race)
  iters <- nrow(fit$tau)
  out <- list()
  for(region in regions) {
    idx <- which(meps$race == region)
    tau_hat_region <- rowMeans(fit$tau[,idx])
    tau_bar <- rowMeans(fit$tau)
    out[[region]] <- data.frame(iteration = iters, region = region, 
                                Delta = tau_hat_region - tau_bar)
...
race_effects_2 <- get_race_effects_2(fitted_bcf_2)
ggplot(race_effects_2, aes(x = Delta, y = region, fill = region)) + 
  stat_halfeye()
```
...
age_effects <- get_age_effects(fitted_bcf_2)
